...
LIST<long long> GRAPH<Type>::Topo_sort(const bool output) const
{
	...
	if (graph_weighted == true)
	{
		cout << "\033[1;33m【警告】该有向图中的弧有权值。拓扑排序中的弧仅代表顶点的先后关系，弧的权值无意义。\033[0m";
		cout << "\033[1;33m在拓扑排序的过程中，有向图中的权值将被忽略\033[0m" << endl;
	}
	TEMP.Repair(0);
	if (vertex_num == 1)
	{
...
template<typename Type>
void GRAPH<Type>::Critical_path() const
{
	if (graph_initialized == false)
	{
		cout << "\033[1;31m【错误】图未初始化，无法输出关键路径\033[0m" << endl;
		return;
	}
	if (graph_directed == false)
	{
		cout << "\033[1;31m【错误】无法在无向图上输出关键路径\033[0m" << endl;
		return;
	}
	if (graph_weighted == false)
	{
		cout << "\033[1;31m【错误】无法在无权图上输出关键路径\033[0m" << endl;
		return;
	}
	if (vertex_num == 0)
	{
		cout << "\033[1;31m【错误】无法在空图上输出关键路径\033[0m" << endl;
		return;
	}
	if (vertex_num == 1)
	{
		cout << "\033[1;31m【错误】有向图中只有一个顶点，不存在弧。无法计算关键路径\033[0m" << endl;
		return;
	}

	//计算初始状态下所有顶点的入度，AOE网中有且仅有一个入度为0的顶点
	long long* indegree = new long long[vertex_num]();//初始化为0
	for (long long i = 1; i <= vertex_num; i++)
	{
		listA_line* scan = vertex_first[i - 1];
	...
	long long indegree_0_pos = 0;
	for (long long i = 1; i <= vertex_num; i++)
	{
		if (indegree[i - 1] == 0)
		{
			indegree_count = indegree_count + 1;
			indegree_0_pos = i;
		}
	}
	if (indegree_count != 1)
	{
		cout << "\033[1;31m【错误】入度为0的顶点不唯一，或不存在入度为0的顶点。无法输出关键路径\033[0m" << endl;
		delete[] indegree;
		return;
	}
	//注意：indegree后面还要用到，不在这里delete

	//计算初始状态下所有顶点的出度，AOE网中有且仅有一个出度为0的顶点
	long long* outdegree = new long long[vertex_num]();//初始化为0
	for (long long i = 1; i <= vertex_num; i++)
	{
		listA_line* scan = vertex_first[i - 1];
		while (scan != nullptr)
		{
			outdegree[i - 1] = outdegree[i - 1] + 1;
			scan = scan->line;
		}
	}
	int outdegree_count = 0;
	for (long long i = 1; i <= vertex_num; i++)
	{
		if (outdegree[i - 1] == 0)
			outdegree_count = outdegree_count + 1;
	}
	if (outdegree_count != 1)
	{
		cout << "\033[1;31m【错误】出度为0的顶点不唯一，或不存在出度为0的顶点。无法输出关键路径\033[0m" << endl;
		delete[] indegree;
		delete[] outdegree;
		return;
	}
	...
	stack_vertex* stack_head = new stack_vertex;
	stack_head->vertex = indegree_0_pos;
	stack_head->next = nullptr;
	long long* sequence = new long long[vertex_num];
	long long count = 0;
	while (stack_head != nullptr)
	{
		long long p_vertex = stack_head->vertex;
		sequence[count] = p_vertex;
		count = count + 1;
		//必须先出栈再入栈，顺序不可以颠倒
		if (stack_head->next == nullptr)
		{
	...
	if (count != vertex_num)
	{
		cout << "\033[1;31m【错误】有向图中有环，无法对有环的有向图输出关键路径\033[0m" << endl;
		delete[] sequence;
		return;
	}

	//事件（顶点）的最早发生时间
	long double* VE = new long double[vertex_num]();//初始化为0
	for (long long i = 1; i <= vertex_num; i++)
	{
		listA_line* scan = vertex_first[sequence[i - 1] - 1];
		while (scan != nullptr)
		{
			if (VE[sequence[i - 1] - 1] + scan->line_weight > VE[scan->vertex - 1])
				VE[scan->vertex - 1] = VE[sequence[i - 1] - 1] + scan->line_weight;
			scan = scan->line;
		}
	}
	//事件（顶点）的最迟发生时间
	long double* VL = new long double[vertex_num];
	for (long long i = 1; i <= vertex_num; i++)
		VL[i - 1] = VE[vertex_num - 1];
	for (long long i = vertex_num; i >= 1; i--)
	{
		listB_line* scan = vertex_firstB[sequence[i - 1] - 1];//这里使用十字链表，而非逆邻接表
		while (scan != nullptr)
		{
			if (VL[scan->vertex_B - 1] - scan->line_weight < VL[scan->vertex_A - 1])
				VL[scan->vertex_A - 1] = VL[scan->vertex_B - 1] - scan->line_weight;
			scan = scan->line_B;
		}
	}
	delete[] sequence;
	...
	struct stack_line
	{
		long long A;//弧尾
		long long B;//弧头
		long double weight;//弧的权重
		long double E;//活动（弧）的最早开始时间
		long double L;//活动（弧）的最迟开始时间
		long double D;//活动（弧）的时间余量
	};
	stack_line** line_info = new stack_line * [line_num];
	for (long long i = 1; i <= line_num; i++)
		line_info[i - 1] = new stack_line;
	count = 0;
	for (long long i = 1; i <= vertex_num; i++)
	{
		listA_line* scan = vertex_first[i - 1];
		...
		{
			line_info[count]->A = i;
			line_info[count]->B = scan->vertex;
			line_info[count]->weight = scan->line_weight;
			count = count + 1;
			scan = scan->line;
		}
	}
	//活动（弧）的最早开始时间
	for (long long i = 1; i <= line_num; i++)
		line_info[i - 1]->E = VE[line_info[i - 1]->A - 1];
	//请忽略visual studio的报错：使用未初始化的内存“* line_info[i - 1]”。这是误报
	//活动（弧）的最迟开始时间
	for (long long i = 1; i <= line_num; i++)
		line_info[i - 1]->L = VL[line_info[i - 1]->B - 1] - line_info[i - 1]->weight;

	/*
	该测试序列会引出一个经典问题：如何判断浮点数为0?
	1,2,3     1'2'0.2,2'3'0.4,1'3'0.6
	弧：1->2   E:0     L:5.55112e-17    D:5.55112e-17
	弧：1->3   E:0     L:1.11022e-16    D:1.11022e-16
	弧：2->3   E:0.2   L:0.2            D:5.55112e-17
	输出结果：该有向图中不存在任何关键路径

	其他例子：
	long double test_A = 0.3;
	long double test_B = 0.9 - 0.6;
	...
	造成这种问题的原因：
	0.3的二进制表示：0.0100  1100  1100 ...（1100无限循环）。0.9和0.6同理。这些小数可以被十进制精确表示，
	但在计算机中，这些小数是无限循环小数。浮点数存在精度限制，从而存在舍入误差。多次计算后，误差会累积
	*/

	//活动（弧）的时间余量
	const long double _EPSILON = 1e-15;
	cout << "关键路径上的弧：" << endl;
	for (long long i = 1; i <= line_num; i++)
	{
		line_info[i - 1]->D = line_info[i - 1]->L - line_info[i - 1]->E;
		if (fabs(line_info[i - 1]->D) < _EPSILON)
			cout << *vertex_id[line_info[i - 1]->A - 1] << " -> " << *vertex_id[line_info[i - 1]->B - 1] << "     ";
	}
	cout << endl;
	...
	for (long long i = 1; i <= vertex_num; i++)
	{
		if ((VE[i - 1] != 0 && fabs(VE[i - 1]) < _EPSILON) || (VL[i - 1] != 0 && fabs(VL[i - 1]) < _EPSILON))
		{
			_WARN = true;
			break;
		}
	...
	if (_WARN == false)
	{
		for (long long i = 1; i <= line_num; i++)
		{
			if ((line_info[i - 1]->E != 0 && fabs(line_info[i - 1]->E) < _EPSILON) ||
				(line_info[i - 1]->L != 0 && fabs(line_info[i - 1]->L) < _EPSILON) ||
				(line_info[i - 1]->D != 0 && fabs(line_info[i - 1]->D) < _EPSILON))
			{
				_WARN = true;
				break;
			}
	...
	if (_WARN == true)
	{
		cout << "\033[1;33m【警告】程序运行的过程中，发现了浮点数的精度限制导致的（高度疑似）浮点数误差。\033[0m";
		cout << "\033[1;33m如十进制数0.3转为二进制后，是一个无限循环小数。而浮点数的精度有限，会造成误差。\033[0m";
		cout << "\033[1;33m当活动的时间余量 < \033[0m" << _EPSILON << "\033[1;33m 时，认为该活动的时间\033[0m";
		cout << "\033[1;33m余量是0，该活动是关键活动\033[0m" << endl;
		cout << "\033[1;33m该警告出现时，输出的关键路径极大概率仍是正确的\033[0m" << endl;
	}

	for (long long i = 1; i <= line_num; i++)
		delete[] line_info[i - 1];
	delete[] line_info;
	return;
}
